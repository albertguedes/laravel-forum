...
@section('title', strtoupper(auth()->profile->username))
@section('description',auth()->profile->bio)
@section('content')
<div class="row" >
    ...
    <div class="mb-5 fs-6 col-12" >
        ...
        <span class="mx-2" ><i class="fas fa-angle-right"></i></span>
        <i class="fas fa-user"></i> {{ auth()->profile->username }}
    </div>

    <div class="pb-3 col-12" >
        <h1 class="pb-3 text-uppercase" >{{ auth()->profile->username }}</h1>
        <h6 class="text-black-50" >Registered since <em>{{ auth()->created_at->format("Y M d") }}</em></h6>
        <hr>
        <p><i class="fas fa-user"></i> {{ auth()->profile->name() }} </p>
        <p><i class="fas fa-envelope"></i> {{ auth()->email }} </p>
        <p><i class="fas fa-venus-mars"></i> {{ auth()->profile->gender ? 'Male' : 'Female' }}</p>
        <p><i class="fas fa-birthday-cake"></i> {{ auth()->profile->birth_date }}</p>
    </div>
